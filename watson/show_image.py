...
def show_image(filename, filepath, situation, face_type, face_pos):
    img = cv.imread(filepath)
    
    font = cv.FONT_HERSHEY_SIMPLEX
    if situation:
        cv.putText(img, situation[0], (20, 1000), font, 2, (0,0,255), 3)
    text_ver_pos = 1200;
    for ele in face_type:
        cv.putText(img, ele, (20, text_ver_pos), font, 2, (0,0,0), 3)
        text_ver_pos = text_ver_pos + 100
    for pos in face_pos:
        cv.rectangle(img, (pos[0], pos[1]), (pos[0] + pos[2], pos[1] + pos[3]), (0,0,0), 3)

    cv.imwrite(filename.replace('jpg','bmp'), img)
